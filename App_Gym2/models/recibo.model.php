...
class facturaModel
{
    ...
    public function Actualizar($idfactura, $cliente, $fecha, $membresia, $monto, $estado) {
        $con = new ClaseConexion();
        $con = $con->ProcedimientoConectar();
        $cadena = "UPDATE `recibos_membresia` SET `cli_id`='$cliente', `fa_fecha`='$fecha',`men_id`='$membresia', `fa_montol_total`='$monto', `estado`='$estado' WHERE id_recibo=$idfactura";
